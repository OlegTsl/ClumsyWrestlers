...
{
    public sealed class ArenaEliminationSystem : IArenaEliminationSystem, IFixedTickable
    {
        ...
        public void FixedTick()
        {
            ...
            for (int i = 0; i < characters.Count; i++)
            {
                ...
                ICharacterActivityState activity = character.GetState<ICharacterActivityState>();
                
                if (!activity.Enabled || character.GetState<ICharacterTransformState>().Position.y >= _arena.EliminationHeight)
                    continue;

                activity.SetEnabled(false);
                
                _events.Publish(new OnCharacterEliminatedEvent(
                    character.CharacterID, character.GetState<ICharacterTeamState>().TeamId));
            }
        }

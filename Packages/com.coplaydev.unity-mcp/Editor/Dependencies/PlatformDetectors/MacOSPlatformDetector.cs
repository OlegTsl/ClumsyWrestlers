    ...
    public class MacOSPlatformDetector : PlatformDetectorBase
    {
        ...
        public override DependencyStatus DetectPython()
        {
            ...
            try
            {
                ...
                // 2. Resolve common absolute locations. Unity Editor processes started
                // from the Hub may not inherit a shell PATH, even when `which` works.
                string[] standardPythonPaths =
                {
                ...
                foreach (string standardPythonPath in standardPythonPaths)
                {
                    if (!File.Exists(standardPythonPath))
                    {
                        continue;
                    }

                    if (TryValidatePython(standardPythonPath, out string version, out string fullPath))
                    {
                        status.IsAvailable = true;
                        status.Version = version;
                        status.Path = fullPath;
                ...
                // 3. Fallback: Try running python directly from PATH
